...
public final class TcpProxyServerForwarder implements ProxyServerForwarder {
    ...
    public TcpProxyServerForwarder(VpnService vpnService, String ip, int mtu,
                                   UidDumper dumper,PackterListener listener) throws IOException {
    ...
    public void forward(byte[] packet, int len, OutputStream output) {
        ...
        //tcp
        if(packterListener != null && localPort != mProxyServer.getPort() && remotePort != 443 && remotePort != 80){
            packterListener.data(NetBareUtils.convertIp(remoteIp),NetBareUtils.convertPort(remotePort),"TCP","");
        }
