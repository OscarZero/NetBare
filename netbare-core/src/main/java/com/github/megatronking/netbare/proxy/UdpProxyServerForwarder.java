...
import com.github.megatronking.netbare.util.PackterListener;
...
public final class UdpProxyServerForwarder implements ProxyServerForwarder {
    ...
    private final PackterListener packterListener;

    public UdpProxyServerForwarder(VpnService vpnService, int mtu, UidDumper dumper,PackterListener listener)
            throws IOException {
    ...
    public void forward(byte[] packet, int len, OutputStream output) {
        ...
        if(packterListener != null && remotePort != 443 && remotePort != 80){
            packterListener.data(NetBareUtils.convertIp(remoteIp),NetBareUtils.convertPort(remotePort),"UDP","");
        }
